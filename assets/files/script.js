...
function ChangeSearchEngine(engine) {
  ...
  document.getElementsByClassName("search-icon-options-selected")[0].classList.remove("search-icon-options-selected");
  document.getElementById(engine + "-icon").classList.add("search-icon-options-selected");
...
const searchIconOptions = document.querySelectorAll('.search-icon-options');

// Add event listeners for search icon options
searchIconOptions.forEach(icon => {
  icon.addEventListener('click', function() {
      searchIconOptions.forEach(icon => icon.classList.remove('active'));
      this.classList.add('active');
      
      const title = this.getAttribute('title').toLowerCase();
      currentSearchEngine = searchEngines[title];
  });
});
